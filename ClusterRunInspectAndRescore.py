...
class JobRunner:
    ...
    def Main(self):
        ...
        os.system(Command)
        ###################################################################
        # rescore with pepnovo
        #first cludge, must copy the Models directory to my new temp place
        Command = "cp -r %s/Models ."%PepNovoDir
        print Command
        sys.stdout.flush()
        os.system(Command)
        PepNovoInputPath = OutputPath
        PepNovoOutputFileName = "%s.rescore.txt"%(self.Stub)
        PepNovoOutputPath = os.path.join(self.ScratchDir, PepNovoOutputFileName)
        for SpectrumFileName in self.SpectrumFileNames:
            ScratchMZXMLPath = os.path.join(self.ScratchDir, SpectrumFileName)
            break #huge cluge to get only one
        Command = "%s/PepNovo_bin -rescore_inspect %s %s -model CID_IT_TRYP -PTMs C+57 -file %s"%(PepNovoDir, PepNovoInputPath, PepNovoOutputPath, ScratchMZXMLPath)
        print Command
        sys.stdout.flush()
        os.system(Command)
        # Copy the file back from scratch:
        FinalOutputPath = os.path.join(ResultsXDir, OutputFileName)
        FinalPepNovoPath = os.path.join(ResultsXDir, PepNovoOutputFileName)
        self.CopyFilePolitely(OutputPath, FinalOutputPath)
        self.CopyFilePolitely(PepNovoOutputPath, FinalPepNovoPath)
